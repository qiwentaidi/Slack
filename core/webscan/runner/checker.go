...
func (c *Checker) Check(target string, pocItem *poc.Poc) (err error) {
	...
	tempRequest, err := clients.ParseRequest(originReq)
	if err != nil {
		c.Result.IsVul = false
		return err
	}
	...
	for _, ruleMap := range pocItem.Rules {
		...
		if len(reqType) > 0 && reqType != string(poc.HTTP_Type) {
			if reqType == poc.TCP_Type || reqType == poc.UDP_Type {
				if nc, err := clients.NewNetClient(rule.Request.Host, clients.Config{
			...
			if len(rule.Request.Raw) > 0 {
				...
				err = clients.Request(target, rule, c.VariableMap)
			}
		}
		...
		pocRstTemp := report.PocResult{IsVul: isMatch}
...
func (c *Checker) checkURL(target string) (string, error) {
	...
	if !util.IsURL(target) {
		if newtarget, err := clients.CheckProtocol(target, clients.DefaultClient()); err == nil {
...
func (c *Checker) UpdateVariableMap(args yaml.MapSlice) {
	for _, item := range args {
		...
		case *proto.UrlType:
			c.VariableMap[key] = util.UrlTypeToString(value)
...
func (c *Checker) newRerverse() *proto.Reverse {
	...
	urlStr := ""
	sub := util.CreateRandomString(12)
