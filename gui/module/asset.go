...
func AssetItem() *fyne.Container {
	...
	c := container.NewAppTabs(
		container.NewTabItem("控股企业", custom.NewTableWithUpdateHeader(&common.HoldAsset, []float32{200, 100, 120, 380, 0}, custom.SuperClick)),
		container.NewTabItem("HUNTER资产数量(资产/1积分)", custom.NewTableWithUpdateHeader(&common.HunterAsset, []float32{400, 200}, custom.SimpleClick)),
		container.NewTabItem("公众号", custom.NewTableWithUpdateHeader(&common.WechatAsset, []float32{100, 200, 100, 100, 300, 0}, custom.SimpleClick)),
	)
	c.SetTabLocation(2)
	start.OnTapped = func() {
		go func() {
