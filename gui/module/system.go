...
// 杀软识别
var (
	AntivirusResult = [][]string{{"进程名称", "PID", "杀软名称"}}
	AuthPatchResult = [][]string{{"微软编号", "补丁编号", "描述", "影响系统", "链接"}}
)

func SystemUI() *fyne.Container {
	entry := widget.NewMultiLineEntry()
	entry.PlaceHolder = "tasklist /svc\n\nor\n\nsysteminfo"
	avcheck := widget.NewButton("杀软识别", func() {
		if entry.Text != "" {
			AntivirusResult = AntivirusResult[:1]
			go AntivirusIdentify(entry.Text)
		}
	})
	patchcheck := widget.NewButton("补丁检测", func() {
		if entry.Text != "" {
			AuthPatchResult = AuthPatchResult[:1]
			go Patch(entry.Text)
		}
	})
	av := custom.NewTableWithUpdateHeader2(&AntivirusResult, []float32{300, 150, 400})
	patch := custom.NewTableWithUpdateHeader2(&AuthPatchResult, []float32{150, 170, 300, 300, 500})
	...
	global.Refresh(c)
	c.SetTabLocation(1)
	v := container.NewVSplit(entry, custom.Frame(c))
	v.Offset = 0.3
	return container.NewBorder(container.NewHBox(avcheck, patchcheck), nil, nil, nil, v)
}

func AntivirusIdentify(tasklist string) {
