    ...
    <CustomTabs>
        ...
        <template #ctrl>
            <el-popover placement="left" :width="630" :height="300" trigger="hover">
                <template #reference>
                    <el-button :icon="QuestionFilled" plain type="warning"
                        v-show="activeName == 'wechat'">错误码详情</el-button>
                </template>
            ...
            <el-button v-for="item in ApiDocsOptions" :icon="ChromeFilled" plain type="info"
                v-show="activeName == item.show" @click="BrowserOpenURL(item.link)">{{ item.label }}</el-button>
        </template>
    </CustomTabs>
