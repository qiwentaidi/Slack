...
async function doRequest(method: string, url: string, parameter: string) {
  ...
  if (parameter != null) {
    let response = await GoFetch(method, url + wechat.accessToken + "&" + parameter, "", [{}], 10, null)
    result.value = JSON.parse(response.Body)
    return
  }
  let response = await GoFetch(method, url + wechat.accessToken, "", [{}], 10, null)
  result.value = JSON.parse(response.Body)
}
...
<template>
  ...
  <div class="my-header" style="margin-top: 10px;">
    <span>Console</span>
    <el-button type="danger" :icon="Delete" circle @click="result = ''" />
  </div>
 
  <pre class="pretty-response" style="height: 45%;"><code>{{ result }}</code></pre>
</template>
