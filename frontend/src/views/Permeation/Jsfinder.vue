...
async function JSFinder() {
  ...
  for (const url of urls) {
    ...
    await AnalyzeAPI(url, baseURL, apiRoute, parseHeaders(config.headers), global.jsfind.authFiled.split(","))
  }
...
<template>
  ...
  <el-dialog v-model="dialog" title="漏洞详情">
    ...
    <span v-if="detail.VulType != '未授权访问'">源目标: <el-link type="primary" @click="BrowserOpenURL(detail.Target)">{{ detail.Target }}</el-link> <br /><br /></span>
